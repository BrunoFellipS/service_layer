...
async def alimentar_lista(lista, url, session):
    # Configura a requisição para buscar os tipos de termos de pagamento
    headers = {
...
@router.get("/")
async def listar_payment_terms():
    ...
    # Chama a função para alimentar a lista de tipos de termos de pagamento
    lista_payment_terms = await alimentar_lista([], f"{base_url}/PaymentTermsTypes?$select=PaymentTermsGroupName,GroupNumber", session)

    return {"payment_terms": lista_payment_terms}
